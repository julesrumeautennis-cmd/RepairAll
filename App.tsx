...
const GlassCard: React.FC<{ children: React.ReactNode, className?: string }> = ({ children, className }) => (
    <div className={`bg-slate-900/40 backdrop-blur-xl border border-slate-700/80 p-8 rounded-2xl shadow-2xl shadow-black/20 ${className}`}>
      {children}
    </div>
);
      ...
      {isAuthenticated && user && (
         <div className="flex items-center gap-4">
            {user.picture ? (
              ...
              <span className="h-10 w-10 rounded-full bg-slate-700 flex items-center justify-center text-white font-bold">{user.name?.charAt(0)}</span>
            )}
            <button
    ...
    return (
      <div className="min-h-screen w-full p-4 md:p-8 flex flex-col items-center justify-center text-slate-200">
        <div className="w-full max-w-md bg-slate-900/40 backdrop-blur-xl border border-slate-700/80 p-8 rounded-2xl shadow-2xl shadow-black/20 text-center animate-fade-in-up">
          ...
          <h1 className="text-3xl font-bold text-white mb-2">Welcome to RepairAll</h1>
          <p className="text-slate-400 mb-8">Your AI-powered assistant for fixing almost anything.</p>
          <button
  ...
  return (
    <div className="min-h-screen text-slate-200 font-sans">
      ...
      <main className="container mx-auto p-4 md:p-8">
        ...
        {!guide && !isLoading && (
          <GlassCard className="max-w-2xl mx-auto animate-fade-in-up">
            <div className="text-center mb-8">
              <h2 className="text-3xl font-bold text-white">Describe Your Broken Item</h2>
              <p className="text-slate-400 mt-2">Provide a picture and a description, and we'll do the rest.</p>
            </div>
              ...
              <div>
                ...
                >
                  <div className="space-y-2 text-center">
                    ...
                    {previewUrl ? (
                      <img src={previewUrl} alt="Preview" className="mx-auto h-48 w-auto rounded-md object-contain" />
                    ) : (
                      ...
                      <>
                        ...
                        <p className="text-xs text-slate-500">PNG, JPG, GIF, WEBP up to 4MB</p>
                      </>
                    )}
                  </div>
                </div>
        ...
        {isLoading && <div className="animate-fade-in-up"><LoadingSpinner message={loadingMessage} /></div>}

        {/* If a guide has been successfully generated, display it. */}
        {guide && (
          <div className="max-w-4xl mx-auto animate-fade-in-up">
            <h2 className="text-4xl font-extrabold text-center text-white mb-6">Your Repair Guide is Ready!</h2>
            {/* Display the safety warning if it exists. */}
            {guide.safetyWarning && (
              <div className="bg-yellow-900/50 border border-yellow-500/50 text-yellow-200 p-4 rounded-lg mb-10 flex items-start gap-3" role="alert">
            ...
            <div className="space-y-12">
              ...
              {guide.steps.map((step, index) => (
                <div 
                  ...
                  className={`flex flex-col md:flex-row gap-8 items-center p-6 bg-slate-900/40 backdrop-blur-xl border border-slate-700/80 rounded-2xl shadow-2xl shadow-black/20 transition-transform duration-300 hover:scale-[1.02] animate-fade-in-up ${index % 2 === 1 ? 'md:flex-row-reverse' : ''}`}
                  // Stagger the animation of each step for a nice cascade effect.
                  style={{ animationDelay: `${index * 150}ms`}}
                >
                  <div className="md:w-1/2 w-full">
                    <img src={step.imageUrl} alt={`Step ${step.stepNumber}`} className="w-full h-auto rounded-xl object-cover aspect-[4/3] bg-slate-800" />
                  </div>
                  <div className="md:w-1/2">
                    <h3 className="text-2xl font-bold text-indigo-400 mb-3">Step {step.stepNumber}</h3>
