...
export const generateRepairGuide = async (
  ...
  const planningResponse = await ai.models.generateContent({
      ...
      config: {
        ...
        responseSchema: {
          type: Type.OBJECT,
          properties: {
            safetyWarning: { type: Type.STRING, description: "A clear, concise safety warning if applicable (e.g., 'Unplug the device before starting.'), otherwise null." },
            steps: {
              type: Type.ARRAY,
              ...
              items: {
                type: Type.OBJECT,
                properties: {
                  stepNumber: { type: Type.INTEGER, description: "The sequential step number." },
                  description: { type: Type.STRING, description: "Textual description of the step. The first step should describe tools needed." },
                  imagePrompt: { type: Type.STRING, description: "A detailed prompt for an AI image generator to create a photorealistic image for this step." }
                },
  ...
  for (const [index, step] of plan.steps.entries()) {
    updateLoadingMessage(`Generating image for step ${index + 1} of ${plan.steps.length}...`);
    ...
    if (!imageGenerationResponse.generatedImages || imageGenerationResponse.generatedImages.length === 0) {
        throw new Error(`Failed to generate an image for step ${step.stepNumber}.`);
    }

    // The image data is returned as a Base64 string.
    const base64ImageData = imageGenerationResponse.generatedImages[0].image.imageBytes;
